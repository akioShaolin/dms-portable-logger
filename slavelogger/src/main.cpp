...
void setup(){bootId=esp_random();snprintf(deviceId,sizeof deviceId,"%012llX",ESP.getEfuseMac());Serial.begin(config::LINK_BAUD,SERIAL_8N1,3,1);panel.begin(config::JBD_BAUD);leds.begin();rtc.begin();uint64_t startEpoch=0;rtc.read(startEpoch);bool fs=LittleFS.begin(false);if(fs){logger.begin(LittleFS,1,bootId,esp_random(),deviceId,DMS_FIRMWARE_VERSION,DMS_GIT_SHA,uint8_t(rtc.quality()),startEpoch);uint8_t event[9],*p=event;put32(p,ESP.getFlashChipSize());put32(p,4194304);*p++=uint8_t(esp_reset_reason());logger.append(LogType::SYSTEM_EVENT,event,p-event);}leds.setServer(fs?LedPattern::GREEN:LedPattern::RED);logQueue=xQueueCreate(12,sizeof(LogItem));xTaskCreatePinnedToCore(panelTask,"panel",7168,nullptr,5,nullptr,1);xTaskCreatePinnedToCore(loggerTask,"logger",4096,nullptr,1,nullptr,0);xTaskCreatePinnedToCore(webTask,"web",6144,nullptr,2,nullptr,0);}
